...
**DKMS 모듈이 커널 7.2 를 지원하는지 먼저 확인한다.** 7.0 용으로 만들어진
드라이버는 빌드가 깨질 수 있다. 예를 들어 NVIDIA 595 는 커널 7.2 가 없앤
`strncpy` 를 아직 써서 빌드되지 않는다. 610 이상을 쓴다.

GPU 드라이버가 빌드되지 않은 채로 재부팅하면 화면이 안 나올 수 있으니,
`dkms status` 로 새 커널 항목이 만들어졌는지 보고 재부팅한다.
